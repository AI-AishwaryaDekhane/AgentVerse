...
from LinguisticAnalysis import generate_linguistic_analysis_report, LinguisticAnalysisRequest
...
#'agent1qtauvslzzjuuel22905s4wjztmf0h9903fwqnmcuvysk0w40f75nua3vplu'
        ...
        if isinstance(item, StartSessionContent):
            ...
            await ctx.send(
                ...
                StructuredOutputPrompt(
                    ...
                    output_schema=LinguisticAnalysisRequest.schema()
                ),
            )
        else:
...
async def handle_structured_output_response(
    ...
    prompt = LinguisticAnalysisRequest.parse_obj(msg.output)
    print(f'-------------------------{prompt}')
    try:
        search_result = await generate_linguistic_analysis_report(prompt.text)
